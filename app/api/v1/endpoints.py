...
from app.services.utils import (prepare_metadata, cleanup_temp_files,
                                validate_request_data, setup_temp_directory)
import os
...
async def run_inference(request: Request, auth=Depends(check_auth)):
    try:
        ...
        temp_images_dir = Path(
            os.environ.get('TEMP_IMAGES_DIR', '/tmp/temp_images'))
        ...
        error_msg = f"Error generating inference for {obj_path}: {str(e)}"
        raise HTTPException(status_code=500, detail=error_msg)
...
async def create_training_job(request: Request, auth=Depends(check_auth)):
    ...
    try:
        ...
        required_fields = ['modelName', 'datasetId', 'datasetPath',
                           'trainingParams']
    ...
    try:
        ...
        return {
            ...
            "elapsedTime": str(
                datetime.now(timezone.utc) - job.started_at
                if job.started_at else None
            ).split('.')[0] if job.started_at else None
        }
...
async def cancel_training_job(job_id: str, auth=Depends(check_auth)):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400,
                detail="Job cannot be cancelled or doesn't exist")
...
@router.get("/models")
async def list_models(auth=Depends(check_auth)):
    ...
    try:
            ...
            "models": [
                {
                    ...
                    "fileSizeMB": round(model.file_size / (1024 * 1024), 2),
                    "description": model.description,
                    "trainedByJob": model.trained_by_job,
                    "datasetInfo": model.dataset_info,
        ...
        print(f"Model listing error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.get("/models/{model_id}")
async def get_model_details(model_id: str, auth=Depends(check_auth)):
    ...
    try:
        ...
        print(f"Model details error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.post("/models/{model_id}/activate")
async def activate_model(model_id: str, auth=Depends(check_auth)):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Model {model_id} is now active for inference"
        }
        ...
        print(f"Model activation error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.get("/models/active")
async def get_active_model(auth=Depends(check_auth)):
    ...
    try:
        ...
        return {
            ...
            "activeModel": {
                ...
                "createdAt": active_model.created_at,
                "description": active_model.description,
                "trainedByJob": active_model.trained_by_job
            }
        }
        
    except Exception as e:
        error_msg = f"Error getting active model: {str(e)}"
        print(f"Active model error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.delete("/models/{model_id}")
async def delete_model(model_id: str, auth=Depends(check_auth)):
    """Delete a model (cannot delete active model unless forced)"""
    try:
        model_manager = ModelManager()
        success = model_manager.delete_model(model_id, force=False)
            ...
            if not model:
                ...
                raise HTTPException(
                    ...
                    detail="Cannot delete active model. "
                           "Set another model as active first."
                )
            else:
                raise HTTPException(
                    status_code=500, detail="Failed to delete model"
                )
        ...
        print(f"Model deletion error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.get("/models/statistics")
async def get_model_statistics(auth=Depends(check_auth)):
    ...
    try:
        ...
        error_msg = f"Error getting model statistics: {str(e)}"
        print(f"Model statistics error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.post("/models/cleanup")
async def cleanup_old_models(auth=Depends(check_auth)):
    """Clean up old model versions, keeping only the latest 5 per model name"""
    try:
        model_manager = ModelManager()
        deleted_count = model_manager.cleanup_old_models(keep_versions=5)
        ...
        error_msg = f"Error during model cleanup: {str(e)}"
        print(f"Model cleanup error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)
...
async def system_health_check(auth=Depends(check_auth)):
    ...
    try:
        ...
        return {
            ...
            "models": {
                "totalModels": model_stats["total_models"],
                "activeModel": active_model.name if active_model else None,
                "storageUsedMB": model_stats["total_storage_mb"]
            },
            "storage": {
...
async def run_system_cleanup(auth=Depends(check_auth)):
    """Run comprehensive system cleanup"""
    try:
        model_manager = ModelManager()
        
        # Clean up old models
        deleted_models = model_manager.cleanup_old_models(keep_versions=5)
        
        # Clean up old checkpoints
        from app.services.error_handling import ErrorHandlingService
        error_handler = ErrorHandlingService()
        cleaned_history = error_handler.cleanup_old_rollback_history(24)
        
        return {
            "success": True,
            "cleanup": {
                "deletedModels": deleted_models,
                "cleanedRollbackHistory": cleaned_history,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
        }
        
    except Exception as e:
        error_msg = f"Error during system cleanup: {str(e)}"
        print(f"System cleanup error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)


@router.get("/training/{job_id}/rollback-history")
async def get_job_rollback_history(job_id: str, auth=Depends(check_auth)):
    """Get rollback history for a training job"""
    try:
        from app.services.error_handling import ErrorHandlingService
        error_handler = ErrorHandlingService()
        
        history = error_handler.get_rollback_history(job_id)
        ...
        error_msg = f"Error getting rollback history: {str(e)}"
        print(f"Rollback history error: {error_msg}", flush=True)
        raise HTTPException(status_code=500, detail=error_msg)
