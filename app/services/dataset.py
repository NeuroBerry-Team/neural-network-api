...
class DatasetService:
    ...
    async def download_and_prepare_dataset(
            self, dataset_info: Dict[str, Any]) -> str:
        """
        Download dataset from MinIO and prepare it for YOLO training
        
        Args:
            dataset_info: Dict containing dataset information from brain-mapper
                - datasetId: int
                - datasetPath: str (S3 path)
                - modelName: str
        ...
        s3_path = dataset_info.get('datasetPath')
        model_name = dataset_info.get('modelName', 'unnamed_model')
        
        print(f"Processing dataset {dataset_id} from S3 path: {s3_path}",
              flush=True)
        
        # Create unique dataset directory
        dataset_uuid = uuid.uuid4().hex[:8]
        dataset_dir = (self.base_dataset_path /
                       f"dataset_{dataset_id}_{dataset_uuid}")
        dataset_dir.mkdir(exist_ok=True)
        
        try:
            # Download dataset from MinIO
            await self._download_dataset_from_minio(s3_path, dataset_dir)
            
            # Extract if it's a zip file
            extracted_dir = await self._extract_dataset_if_needed(dataset_dir)
            
            # Validate and organize dataset structure
            organized_dir = await self._organize_dataset_structure(
                extracted_dir)
            ...
            print(f"Dataset prepared successfully: {yaml_config_path}",
                  flush=True)
    ...
    async def _download_dataset_from_minio(self, s3_path: str,
                                           target_dir: Path) -> None:
        """Download dataset from MinIO S3 storage"""
        try:
            minio_client = get_minio_client()
            bucket_name = os.environ.get('S3_BUCKET_DATASETS', 'datasets')
            
            # Parse S3 URL to extract object path
            if s3_path.startswith('s3://'):
                # Remove s3:// prefix and extract bucket and path
                s3_url = s3_path[5:]  # Remove 's3://'
                if '/' in s3_url:
                    url_bucket, object_path = s3_url.split('/', 1)
                    # Use the bucket from URL if different from env
                    if url_bucket != bucket_name:
                        print(f"Using bucket from URL: {url_bucket} "
                              f"instead of env: {bucket_name}", flush=True)
                        bucket_name = url_bucket
                else:
                    object_path = s3_url
            else:
                # Remove leading slash if present
                object_path = s3_path.lstrip('/')
            
            print(f"Downloading from MinIO - Bucket: {bucket_name}, "
                  f"Object: {object_path}", flush=True)
            
            # Check if it's a single file or a folder
            try:
                # Try to get object stat to see if it's a single file
                obj_stat = minio_client.stat_object(bucket_name, object_path)
                print(f"Found single file: {object_path} ({obj_stat.size} bytes)", flush=True)
                
                # Determine file extension from S3 path
                file_extension = Path(object_path).suffix or '.zip'
                download_file_path = target_dir / f"dataset{file_extension}"
                
                # Download the single file
                minio_client.fget_object(bucket_name, object_path,
                                         str(download_file_path))
                
                print(f"Dataset downloaded to: {download_file_path}", flush=True)
                
            except Exception:
                # If stat fails, it might be a folder - try to list and download all files
                print(f"Object appears to be a folder, downloading all contents", flush=True)
                
                objects = minio_client.list_objects(bucket_name, prefix=object_path, recursive=True)
                downloaded_files = 0
                
                for obj in objects:
                    if not obj.object_name.endswith('/'):  # Skip folder entries
                ...
                print(f"Downloaded {downloaded_files} files from folder", flush=True)
                
        except Exception as e:
            raise Exception(f"Failed to download dataset from MinIO: {str(e)}")
    ...
    async def _organize_dataset_structure(self, dataset_dir: Path) -> Path:
        ...
        if self._is_full_yolo_format(dataset_dir):
            print("Dataset already in full YOLO format with train/val splits",
                  flush=True)
            return dataset_dir
        
        # Check if it's in basic YOLO format (just images/ and labels/ folders)
        if self._is_basic_yolo_format(dataset_dir):
            print("Dataset in basic YOLO format, creating train/val splits", 
                  flush=True)
        ...
        print(f"Dataset organized into YOLO format: {organized_dir}", 
              flush=True)
    ...
    def _is_full_yolo_format(self, dataset_dir: Path) -> bool:
        ...
        return all(dir_path.exists() and dir_path.is_dir()
    ...
    def _is_basic_yolo_format(self, dataset_dir: Path) -> bool:
        ...
        return (images_dir.exists() and images_dir.is_dir() and
    ...
    async def _create_train_val_split(self, dataset_dir: Path) -> Path:
        ...
        await self._copy_split_files(dataset_dir, split_dir,
                                     train_images, "train")
        await self._copy_split_files(dataset_dir, split_dir, 
                                     val_images, "val")
        
        print(f"Created train/val split: {len(train_images)} train, "
              f"{len(val_images)} val images", flush=True)
        
        return split_dir
    
    async def _copy_split_files(self, source_dir: Path, target_dir: Path,
                                image_files: List[Path], split: str) -> None:
    ...
    async def _organize_files_to_yolo(self, source_dir: Path, 
                                      target_dir: Path) -> None:
        ...
        await self._copy_files_with_labels(
            val_images, label_files, target_dir, "val")
    
    async def _copy_files_with_labels(self, image_files: List[Path], 
                                      label_files: List[Path], 
                                      target_dir: Path, split: str) -> None:
        ...
        for image_file in image_files:
            ...
            label_name = image_file.stem + ".txt"
            corresponding_labels = [lf for lf in label_files 
                                   if lf.name == label_name]
        ...
        if class_files:
            ...
            try:
                ...
                print(f"Error reading classes file: {e}. "
                      f"Falling back to label detection", flush=True)
    ...
    async def _create_yolo_config(self, dataset_dir: Path,
        ...
        config_file = (config_dir /
                       f"{model_name}_dataset_{dataset_id}_{config_uuid}.yaml")
    ...
    def cleanup_dataset(self, yaml_config_path: str) -> None:
        ...
        try:
            ...
            try:
                ...
                print(f"Warning: Could not parse dataset path from YAML: {parse_error}", flush=True)
            ...
            if dataset_dir and dataset_dir.exists():
                ...
                if datasets_base_str in str(dataset_dir):
                    try:
                        ...
                        shutil.rmtree(dataset_dir)
                        print(f"Cleaned up dataset directory: {dataset_dir}", flush=True)
                        
                        # Also check for any related temp files
                        dataset_parent = dataset_dir.parent
                        if dataset_parent.name == "datasets":
                            dataset_pattern = dataset_dir.name.split('_')[:-1]  # Remove UUID part
                            for related_dir in dataset_parent.glob('_'.join(dataset_pattern) + '_*'):
                                if related_dir != dataset_dir and related_dir.is_dir():
                                    try:
                                        shutil.rmtree(related_dir)
                                        print(f"Cleaned up related dataset: {related_dir}", flush=True)
                                    except (FileNotFoundError, PermissionError) as e:
                                        print(f"Warning: Could not clean up {related_dir}: {e}", flush=True)
                    except (FileNotFoundError, PermissionError) as cleanup_error:
                        print(f"Warning: Could not clean up dataset directory {dataset_dir}: {cleanup_error}", flush=True)
                                
        except Exception as e:
            # Don't fail on cleanup errors, especially during cancellation
            print(f"Note: Cleanup encountered issues (normal during cancellation): {str(e)}", flush=True)
            
    def cleanup_training_artifacts(self, model_name: str, 
            ...
            if config_dir.exists():
                ...
                for config_file in config_dir.glob(pattern + ".yaml"):
                    try:
                        ...
                        print(f"Warning: Could not clean up config {config_file}: {e}", 
                              flush=True)
                ...
                if datasets_dir.exists():
                    for dataset_dir in datasets_dir.glob(f"dataset_{dataset_id}_*"):
                        if dataset_dir.is_dir():
                            try:
                                shutil.rmtree(dataset_dir)
                                print(f"Cleaned up dataset: {dataset_dir}", 
                                      flush=True)
                            except (FileNotFoundError, PermissionError) as e:
                                print(f"Warning: Could not clean up dataset {dataset_dir}: {e}", 
                                      flush=True)
                            
        except Exception as e:
            print(f"Note: Training artifact cleanup encountered issues: {str(e)}", 
                  flush=True)
